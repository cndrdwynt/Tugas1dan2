...
def draw_avatar_and_ui(final_output, state, assets, current_head_img, current_body_img,
    ...
    # Filter Mode Display (Tugas 1)
    from utils import get_filter_mode_name
    filter_mode = state.get("filter_mode", 0)
    filter_name = get_filter_mode_name(filter_mode)
    filter_txt = f"Filter: {filter_name} (0-4 untuk ubah)"
    final_output = draw_fancy_text(final_output, filter_txt, cfg.UI_FILTER_POS, cfg.UI_FONT, cfg.UI_FILTER_SCALE, cfg.UI_TEXT_COLOR, cfg.UI_FILTER_THICKNESS, cfg.UI_BG_COLOR, cfg.UI_BG_ALPHA, cfg.UI_PADDING)
    
    # Color Detection Display (Tugas 2)
    from utils import get_color_name_indonesian, draw_color_detection_overlay
    color_detect_enabled = state.get("color_detect_enabled", False)
    if color_detect_enabled:
        color_mode = state.get("color_detect_mode", cfg.COLOR_DETECT_GREEN)
        color_name = get_color_name_indonesian(color_mode)
        color_txt = f"Color: {color_name} (g/h/r/y, c=toggle)"
        color = (0, 255, 0)
        final_output = draw_fancy_text(final_output, color_txt, cfg.UI_COLOR_DETECT_POS, cfg.UI_FONT, cfg.UI_COLOR_DETECT_SCALE, color, cfg.UI_COLOR_DETECT_THICKNESS, cfg.UI_BG_COLOR, cfg.UI_BG_ALPHA, cfg.UI_PADDING)
        
        # Draw detection overlay
        detection_result = state.get("color_detection_result")
        if detection_result and detection_result["area"] > cfg.COLOR_DETECT_MIN_AREA:
            final_output = draw_color_detection_overlay(
                final_output,
                detection_result["contour"],
                detection_result["center"],
                detection_result["color_name"],
                detection_result["area"]
            )
    else:
        color_txt = "Color Detection: OFF (tekan 'c')"
        color = (128, 128, 128)
        final_output = draw_fancy_text(final_output, color_txt, cfg.UI_COLOR_DETECT_POS, cfg.UI_FONT, cfg.UI_COLOR_DETECT_SCALE, color, cfg.UI_COLOR_DETECT_THICKNESS, cfg.UI_BG_COLOR, cfg.UI_BG_ALPHA, cfg.UI_PADDING)
